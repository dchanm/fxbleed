...
function db_heartbeat(host, version) {
  // valid heartbeat + dummy heartbeat to detect GnuTLS
  // |TLS_RECORD|HB_HDR|HB_DATA|TLS_RECORD|HB_HDR|
  let payload = new Uint8Array(HDR_LENGTH + HB_HDR + HB_LENGTH +
                               HDR_LENGTH + HB_HDR)

  let first = heartbeat(HB_LENGTH, host, version)
  payload.set(first)
  let second = heartbeat(0, "", version)
  payload.set(second, first.length)

  return payload.buffer
}

function heartbeat(length, host, version) {
  let payload = new Uint8Array(HB_HDR + length);
  let pos = 0;
  ...
  for (let i = 0; i < length && i < host.length; i++) {
  ...
  return ssl_record(HEARTBEAT_RECORD_TYPE, payload, version);
}

function client_hello(version) {
  // larger than needed
  ...
  return ssl_record(HANDSHAKE_RECORD_TYPE, data, version).buffer;
}

function ssl_record(type, data, version) {
  let record = new Uint8Array(HDR_LENGTH + data.byteLength);
  record[0] = type;
...
function connect(url) {
  ...
  try {
    ...
    sock.ondata = function (evt) {
      ...
      if (!hdr) {
        if (!buffer.has(HDR_LENGTH)) {
          return;
        }

        hdr = parseHeader(buffer.get(HDR_LENGTH));
        /*
         * We request TLS 1.2 initially, but the server may
         * respond with a lower version. We assume the server
         * doesn't change the TLS version after the handshake
         */
        version = hdr.version;
      }
        ...
        switch (hdr.type) {
          case HANDSHAKE_RECORD_TYPE:
            if (!t) {
              ...
              sock.send(db_heartbeat(host + "\xFFFXBleed", version));
    ...
    sock.onopen = function () {
      ...
      sock.send(client_hello(version));
    }
