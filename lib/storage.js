...
exports.initStorage = function() {
  ...
  if (!storage.wasVulnerable) {
    storage.wasVulnerable = {};
  }
...
function clearStorage() {
  ...
  // Don't clear wasVulnerable, we want to keep track of any sites that are
  // vulnerable to improve our coverage
}
...
function cacheResponse(domain, response) {
  ...
  // If the domain is vulnerable, cache that so we can remember even after
  // they patch
  if (response.code == 0) {
    storage.wasVulnerable[domain] = true;
  }
}
...
exports.wasVulnerable = function(domain) {
  return storage.wasVulnerable.hasOwnProperty(domain);
}
