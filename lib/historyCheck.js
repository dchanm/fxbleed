...
function checkHistory(worker, historyList, index) {
  ...
  checkDomain(domain, function callback(response) {
    ...
    // Don't keep doing lookups if they've closed the check page
...
function doHistoryCheck(worker) {
  ...
  search(
    ...
    {
      "sort": "visitCount",
      "count": 1000 // TODO tune (abitrary), if we fix jank below this won't be needed
    }
